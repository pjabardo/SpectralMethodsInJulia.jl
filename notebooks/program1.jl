...
# ╔═╡ 39353f43-4561-4791-b445-ebeef8930971
begin
	using CairoMakie
	using SparseArrays
	using LinearAlgebra
...
# ╔═╡ 7345a56a-4080-4d7d-9936-a210e36a3c3d
using ToeplitzMatrices

# ╔═╡ 59b51ae0-4f21-11ed-3af6-91960bf379b3
md"""
# Program 1 

Program 1 in chapter 1
"""

# ╔═╡ a77ab87f-4230-4f93-ab61-d7c69b763af0
Nvec = 2 .^ (3:12)

# ╔═╡ 9ad56aad-3b10-40a7-a1f8-15a5dd88385c
function calcerror(N)
	h = 2π/N; x = -π .+ (1:N) * h
	u = @. exp(sin(x))
	du = @. cos(x) * u
	e = ones(N)
	D = sparse(1:N, [2:N; 1], (2/3)*e, N, N) - 
		sparse(1:N, [3:N; 1; 2], (1/12)*e, N, N) 
	D = (D-D') / h
...
# ╔═╡ ad6d2697-def5-4ecf-841a-528d88fba672
let
	err = calcerror.(Nvec)
	fig, ax, plt = scatter(Nvec, err, axis=(yscale=log10,xscale=log10, xlabel="N",
							ylabel="Error", title="4th degree with Circulan Toeplitz matrix"))

	idx = length(Nvec) ÷ 2
	e0 = err[idx]/100; n0 = Nvec[idx]
	C = e0 / n0^(-4)

	lines!(ax, Nvec, C*Nvec.^(-4), linestyle=:dash)
	annotations!(ax, ["N⁻⁴"], [Point(n0*1.0, e0)])
	
	fig
end

# ╔═╡ c62d29da-13cd-43bf-bc69-eed808729caa
md"""
## Using better types of matrix

In the program above, we used sparse matrix to represent the differentiation operator. The matrix is a circulant Toeplitz matrix and Julia has a package to represent this kind of matrix efficiently. Below we repeat the program using `Circulant`.

"""

# ╔═╡ e9d5407d-5e81-45ce-8e6c-c7a97095b2f0
function calcerror_toeplitz(N)
	h = 2π/N; x = -π .+ (1:N) * h
	u = @. exp(sin(x))
	du = @. cos(x) * u
	e = ones(N)
...
# ╔═╡ 937d5a89-2b02-4159-8315-b3982f766343
let
	err = calcerror_toeplitz.(Nvec)
	fig, ax, plt = scatter(Nvec, err, axis=(yscale=log10,xscale=log10, xlabel="N",
							ylabel="Error", title="4th degree with Circulan Toeplitz matrix"))
...
julia_version = "1.8.2"
manifest_format = "2.0"
project_hash = "917453d9933952295b09b8732691a8aac66026eb"
